...
*/
	...
	public ArrayList<School> getIdByName(String name) {
		...
		ArrayList<School> returnSchools = new ArrayList<School>();
		for(int i = 0; i < schools.size(); i++) {
			if(schools.get(i).getSchool().equals(name))
				returnSchools.add(schools.get(i));
		}
    ...
    public void addNewSchoolEmphasis(String school, String[] emphasis){
    	for(int i=0; i<emphasis.length; i++){
        	if(dummydatabase.university_addUniversityEmphasis(school, emphasis[i]) == -1)
        		throw new IllegalArgumentException("emphasis '"+emphasis[i]+"' count not be added to school: "+ school+".");
    	}
